...
class RenderizadorGraficosConicas:
    ...
    def _draw_hyperbola(self) -> None:
        ...
        Se usa `np.where` para evitar valores imaginarios al tomar raíces.
        """
        ...
        if self.canonical_data.get('type') == 'hipérbola_vertical' or (self.coefficients['A'] > 0 and self.coefficients['B'] < 0):
            ...
            # Ecuación: (y-k)^2/a^2 - 1 = (x-h)^2/b^2  => despejamos x
            valid = (y - k)**2 / a**2 - 1
            valid = valid * b**2
            # Evitar raíces de números negativos sustituyéndolos por NaN
            valid = np.where(valid >= 0, valid, np.nan)
            self.ax.plot(h + np.sqrt(valid), y, color='#2ca02c', linewidth=2)
            self.ax.plot(h - np.sqrt(valid), y, color='#2ca02c', linewidth=2)
        else:
            ...
            valid = valid * b**2
            valid = np.where(valid >= 0, valid, np.nan)
            self.ax.plot(x, k + np.sqrt(valid), color='#2ca02c', linewidth=2)
            self.ax.plot(x, k - np.sqrt(valid), color='#2ca02c', linewidth=2)
    ...
    def _draw_parabola(self) -> None:
        ...
        if orientation == 'vertical':
            x = np.linspace(h - 12, h + 12, 800)
            y = k + ((x - h) ** 2) / (4 * p)
            self.ax.plot(x, y, color='#d62728', linewidth=2)
            self.ax.plot(h, k, 'ro', label=f'Vertice ({h:.2f}, {k:.2f})')
            # Directriz y = k - p
            self.ax.axhline(k - p, color='#9467bd', linestyle='--', linewidth=1.5, label=f'Directriz y={k - p:.2f}')
        else:
            y = np.linspace(k - 12, k + 12, 800)
            x = h + ((y - k) ** 2) / (4 * p)
            self.ax.plot(x, y, color='#d62728', linewidth=2)
            self.ax.plot(h, k, 'ro', label=f'Vertice ({h:.2f}, {k:.2f})')
            # Directriz x = h - p
